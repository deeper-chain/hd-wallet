...
use bitcoin::hash_types::{PubkeyHash as PubkeyHashType, ScriptHash as ScriptHashType};
use bitcoin::network::constants::Network;
use bitcoin::util::{
    address::{Error as BtcAddressError, Payload, Payload::PubkeyHash, WitnessVersion},
    base58,
};
...
impl Address for BtcForkAddress {
    fn from_public_key(public_key: &TypedPublicKey, coin: &CoinInfo) -> Result<String> {
        ...
        } else if seg_str == "P2TR" {
            BtcForkAddress::p2tr(&public_key.to_bytes(), &network)?.to_string()
        } else {
...
impl BtcForkAddress {
    ...
    pub fn p2tr(pub_key: &[u8], network: &BtcForkNetwork) -> Result<BtcForkAddress> {
        ...
        let addr = BtcAddress::p2tr(secp, pub_key.inner.into(), None, Network::Bitcoin);
        Ok(BtcForkAddress {
            payload: addr.payload,
            network: network.clone(),
        })
...
impl FromStr for BtcForkAddress {
    ...
    fn from_str(s: &str) -> result::Result<BtcForkAddress, BtcAddressError> {
        ...
        if let Some(network) = bech32_network {
            ...
            if expected != variant {
                return Err(BtcAddressError::InvalidBech32Variant {
                    expected,
                    found: variant,
                });
            }
